...
test_name = "test_1_p1_p99_b32_journal_7bands"
...
for img in invalid_images_list:
  X_train_list.remove(img)
  print(f"Removed {img} from X_train_list")
...
# for img in invalid_images_list:
#   X_val_list.remove(img)
#   print(f"Removed {img} from X_val_list")
...
# for img in invalid_images_list:
#   X_test_list.remove(img)
#   print(f"Removed {img} from X_test_list")
...
########## ADDING CLOUD PROBABILITY ##########
# Training set
X_train_cp = []
invalid_images_list_cp = []

for img_name in X_train_list:
  try:
    # read sentinel-2 cloud map probability
    img_xarr_s2_cp = xarray.open_rasterio(f"{dataset_s2_cp_map_dir}/{img_name}")
    img_xarr_s2_cp_t = img_xarr_s2_cp.transpose('y','x','band')

    # append
    X_train_cp.append(img_xarr_s2_cp_t)
  except Exception as ex:
    print(f"Exception: {ex}... Ignoring img {img_name}")
    invalid_images_list_cp.append(img_name)
...
for img_name in X_val_list:
  try:
    ...
    # append
    X_val_cp.append(img_xarr_s2_cp_t)
  except Exception as ex:
    print(f"Exception: {ex}... Ignoring img {img_name}")
    invalid_images_list_cp.append(img_name)
...
for img_name in X_test_list:
  try:
    ...
    # append
    X_test_cp.append(img_xarr_s2_cp_t)
  except Exception as ex:
    print(f"Exception: {ex}... Ignoring img {img_name}")
    invalid_images_list_cp.append(img_name)
...
X_train_cp = X_train_cp / np.array([100]) # normalization between 0 and 1
X_train = np.concatenate([X_train, X_train_cp], axis = 3) # concatenation with other bands

X_val_cp = X_val_cp / np.array([100]) # normalization between 0 and 1
X_val = np.concatenate([X_val, X_val_cp], axis = 3) # concatenation with other bands

X_test_cp = X_test_cp / np.array([100]) # normalization between 0 and 1
X_test = np.concatenate([X_test, X_test_cp], axis = 3) # concatenation with other bands
...
print("Unet...")
model_unet = UNet(input_size=(256,256,7), lr=0.0001)
model_unet.summary()
...
model_unet.fit(train, steps_per_epoch=88, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
...
# Train U-Net with generator
print("A-Unet...")
model_am_unet = UNetAM(input_size=(256,256,7), lr=0.0001, batch_size=32)
...
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_am_unet.fit(train, batch_size=32, steps_per_epoch=88, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
...
print("Resnet...")
model_resnet50segnet = ResNet50SegNet(input_size=(256,256,7), lr=0.0001)
...
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_resnet50segnet.fit(train, batch_size = 32, steps_per_epoch=88, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
np.save(f'{current_test_result_dir}/{resnet50segnet_sent2_history}', model_resnet50segnet.history.history)
print("Resnet...: Done.")

# FCN32
print("FCN32...")
model_fcn_32 = fcn_32(input_size=(256,256,7), lr = 0.0001)
...
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_fcn_32.fit(train, batch_size = 32, steps_per_epoch=88, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model_fcn32])
...
def compute_accuracy(model, images, labels):
  ...
  for i in range(len(images)):
    y_hat = model.predict(images[i].reshape(1,256,256,7)) > 0.5
...
resnet_segnet = ResNet50SegNet(input_size=(256,256,7))
...
metrics_sent2_pd.to_csv(f'{current_test_result_dir}/metrics_s2_s1_cp.csv')
