...
# 4000 images     -> 2800/600/600
# 1/2 training    -> 1400/2000/600
# 1/4 training    -> 700/2700/600

dataset_name = "dataset_02_mean_2019"
test_name = "test_1_p1_p99_b32_journal"
...
# 1/4 SETTING
# X_train_list, X_test_val_list, y_train_list, y_test_val_list = train_test_split(sent2_images_list[:NUM_IMGS], forest_images_list[:NUM_IMGS], test_size=0.825, random_state=42)
# X_val_list, X_test_list, y_val_list, y_test_list = train_test_split(X_test_val_list, y_test_val_list, test_size=0.18, random_state=42)
...
for img_name in y_train_list:
  ...
  # img_noisy_xarr = xarray.open_rasterio(f"{dataset_noisy_map_dir}/{img_name}")
  # img_noisy_xarr_t = img_noisy_xarr.transpose('y','x','band')
  # img_noisy_xarr_clf = (img_noisy_xarr_t > 1) * 1 # pixel changes more than once in 4 years
  # print(img_noisy_xarr_clf)

  w = calculate_sample_weights(img_xarr_clf)
  # w = calculate_sample_weights(img_xarr_clf, img_noisy_xarr_clf) # noisy case
...
# Normalization zscore = (x -u) / sigma
  ...
  X_train = (X_train - mean_sent2) / std_sent2
  X_val = (X_val - mean_sent2) / std_sent2
  X_test = (X_test- mean_sent2) / std_sent2

# Normalization (x-np.min(x))/(np.max(x)-np.min(x))
else:
  print("NORMALIZATION MIN-MAX", flush=True)
  X_train = (X_train - min_sent2) / (max_sent2 - min_sent2)
  X_val = (X_val - min_sent2) / (max_sent2 - min_sent2)
  X_test = (X_test - min_sent2) / (max_sent2 - min_sent2)
...
def UNet(trained_weights = None, input_size = (256,256,4), drop_rate = 0.25, lr=0.0001):
...
save_model = ModelCheckpoint(f'{current_test_result_dir}/{unet_sent2_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_unet.fit(train, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
np.save(f'{current_test_result_dir}/{unet_sent2_history}', model_unet.history.history)

# ATTENTION UNET
# Train U-Net with generator
model_am_unet = UNetAM(input_size=(256,256,4), lr=0.0005, batch_size=32)
model_am_unet.summary()

save_model = ModelCheckpoint(f'{current_test_result_dir}/{am_unet_sent2_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_am_unet.fit(train, batch_size=32, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
np.save(f'{current_test_result_dir}/{am_unet_sent2_history}', model_am_unet.history.history)
...
save_model = ModelCheckpoint(f'{current_test_result_dir}/{resnet50segnet_sent2_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_resnet50segnet.fit(train, batch_size = 32, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
np.save(f'{current_test_result_dir}/{resnet50segnet_sent2_history}', model_resnet50segnet.history.history)

# FCN32
model_fcn_32 = fcn_32(lr = 0.0001)
model_fcn_32.summary()

save_model_fcn32 = ModelCheckpoint(f'{current_test_result_dir}/{fcn32_sent2_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_fcn_32.fit(train, batch_size = 32, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model_fcn32])
np.save(f'{current_test_result_dir}/{fcn32_sent2_history}', model_fcn_32.history.history)
...
# load models
unet = load_model(f'{current_test_result_dir}/{unet_sent2_model_name}', compile=False)
attention_unet = load_model(f'{current_test_result_dir}/{am_unet_sent2_model_name}', compile=False)
resnet_segnet = ResNet50SegNet(input_size=(256,256,4))
resnet_segnet.load_weights(f'{current_test_result_dir}/{resnet50segnet_sent2_model_name}')
fcn32 = load_model(f'{current_test_result_dir}/{fcn32_sent2_model_name}', compile=False)
...
metrics_sent2_pd.to_csv(f'{current_test_result_dir}/metrics_sent2.csv')
...
# X_train_list, X_test_val_list, y_train_list, y_test_val_list = train_test_split(sent1_images_list[:NUM_IMGS], forest_images_list[:NUM_IMGS], test_size=0.825, random_state=42)
# X_val_list, X_test_list, y_val_list, y_test_list = train_test_split(X_test_val_list, y_test_val_list, test_size=0.18, random_state=42)
...
# Normalization zscore = (x -u) / sigma
  ...
  X_train = (X_train - mean_sent1) / std_sent1
  X_val = (X_val - mean_sent1) / std_sent1
  X_test = (X_test- mean_sent1) / std_sent1

# Normalization (x-np.min(x))/(np.max(x)-np.min(x))
else:
  X_train = (X_train - min_sent1) / (max_sent1 - min_sent1)
  X_val = (X_val - min_sent1) / (max_sent1 - min_sent1)
  X_test = (X_test - min_sent1) / (max_sent1 - min_sent1)
...
def UNet2D(trained_weights = None, input_size = (256,256,2), drop_rate = 0.25, lr=0.0001):
...
def UNetAM2D(trained_weights = None, input_size = (256,256,2), drop_rate = 0.25, lr=0.0001, filter_base=16):
...
save_model = ModelCheckpoint(f'{current_test_result_dir}/{unet_sent1_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_unet2D.fit(train, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
np.save(f'{current_test_result_dir}/{unet_sent1_history}', model_unet2D.history.history)

# Attention UNET
# Train U-Net with generator
model_am_unet2D = UNetAM2D(input_size=(256,256,2), lr=0.0005)
model_am_unet2D.summary()
save_model = ModelCheckpoint(f'{current_test_result_dir}/{am_unet_sent1_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_am_unet2D.fit(train, batch_size=32, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
np.save(f'{current_test_result_dir}/{am_unet_sent1_history}', model_am_unet2D.history.history)

# ResNet50SegNet
# Train ResNet50SegNet with generator
model_resnet50segnet2D = ResNet50SegNet2D(input_size=(256,256,2), lr=0.0001)
model_resnet50segnet2D.summary()
save_model = ModelCheckpoint(f'{current_test_result_dir}/{resnet50segnet_sent1_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
model_resnet50segnet2D.fit(train, batch_size = 32, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model])
np.save(f'{current_test_result_dir}/{resnet50segnet_sent1_history}', model_resnet50segnet2D.history.history)

# FCN32
fcn_32_2d = fcn_32_2d(lr = 0.0001)
fcn_32_2d.summary()
save_model_fcn32_2d = ModelCheckpoint(f'{current_test_result_dir}/{fcn32_sent1_model_name}', monitor='val_loss',verbose=1, save_best_only=True)
train = trainGenerator(32, X_train_images, y_train_images, w_train_images, data_gen_args, save_to_dir=None)
fcn_32_2d.fit(train, batch_size = 32, steps_per_epoch=STEP_PER_EPOCHS, epochs=50, validation_data = (X_val_images, y_val_images), callbacks=[save_model_fcn32_2d])
np.save(f'{current_test_result_dir}/{fcn32_sent1_history}', fcn_32_2d.history.history)
...
# load models
unet = load_model(f'{current_test_result_dir}/{unet_sent1_model_name}',compile=False)
attention_unet = load_model(f'{current_test_result_dir}/{am_unet_sent1_model_name}', compile=False)
resnet_segnet = ResNet50SegNet2D(input_size=(256,256,2))
resnet_segnet.load_weights(f'{current_test_result_dir}/{resnet50segnet_sent1_model_name}')
# resnet_segnet.load_weights(f'{current_test_result_dir}/{resnet50segnet_sent1_model_name}', by_name=True, skip_mismatch=True)
fcn32 = load_model(f'{current_test_result_dir}/{fcn32_sent1_model_name}', compile=False)
...
metrics_sent1_pd.to_csv(f'{current_test_result_dir}/metrics_sent1.csv')
